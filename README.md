...
Milestones **M0–M3** (planning, foundation, local vertical slice, live Best Buy validation) are **done**. **M4** (MVP product workflows) is **done**: discovery, listing review, notifications (read API + evaluators), display currency, scheduled scrape job (T3.5), digest email (T3.6), settings page (T4.2), and account delete (T4.3). **M5** in progress: T5.1 benchmark harness and T5.2 Shopify retailers (`palmisleskate`, `tikiroomskate`) are **done** (`docs/benchmarks/`). Human setup **H4** (Resend) and **H5** (Render/Vercel/GitHub Actions) and deployment docs (**T6.1**) are **done** — production URLs in [`docs/DEPLOYMENT.md`](docs/DEPLOYMENT.md). See [`docs/ROADMAP.md`](docs/ROADMAP.md) §15 for the recommended next tasks.
