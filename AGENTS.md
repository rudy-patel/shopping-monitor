...
| Playwright e2e | `make test-e2e` or `cd frontend && npm run test:e2e` | Auto-starts backend :8000 + frontend :3000 via Playwright `webServer`. Runs **Desktop Chrome** and **Mobile Chrome** projects. Requires Supabase creds in `backend/.env` (`make setup-integration-env`). CI: `playwright-e2e` job runs when GitHub Actions Supabase secrets are set; otherwise skips with a warning. |
...
| Production demo seed | `cd backend && source venv/bin/activate && python scripts/seed_demo_data.py --email <user> --dry-run` then `--apply` | Service-role writes a varied catalog (products, price history, notifications) for an existing Google-auth user on production Supabase. Manifest at `backend/scripts/.demo_seed_manifest.json` enables `--cleanup`. Never uses `fixtures.local` URLs; sets `email_sent_at` on seeded notifications to avoid digest sends. `--refresh-timestamps` updates `last_refresh_at` / `last_scraped_at` for seeded products only (catalog-title fallback when manifest is missing). Not for CI (`--apply` / `--refresh-timestamps` refused when `CI=true`). |
