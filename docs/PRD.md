...
- **U-VIEW-3.** Each product row shows: title and brand on one line, retailer of best current price (with a small bundled icon for supported retailer slugs), best current price (in display currency), 30-day trend chip (compact label on list rows; full window copy on detail), alternate retailers count, last-refreshed timestamp, refresh control (icon on `sm+`; kebab menu on all breakpoints), and a kebab menu. V1 list surfaces are typography-first — no in-app product images; users open the retailer PDP via listing links when they want to see photos.
