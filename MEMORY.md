...
**Verification:** `ruff check .`, `pytest -m "not integration"` with `SCRAPER_MODE=fixtures`. H4 done; sandbox live smoke recipient `rutvik@ualberta.ca` (human inbox check after one `--live` send).
